...
def main():
    ...
    if args.do_train:
        ...
        #find the tag vocab ids
        # ids_for_tag_vacab = tokenizer.convert_tokens_to_ids(TAGS_TOKENS_LIST)
        ids_for_tag_vacab = torch.load('./ids_for_tag_vacab.pth')
        tag_id=tokenizer.convert_tokens_to_ids(tokenizer.tokenize('[TAG]'))
        print('[TAG] id: ',tag_id)
        print('last id in ids_for_tag_vocab: ',29659 in ids_for_tag_vacab)
        print('[TAG] id in ids_for_tag_vocab: ',tag_id in ids_for_tag_vacab)
        # torch.save(ids_for_tag_vacab, './ids_for_tag_vacab.pth')
        # print(ids_for_tag_vacab)
        # print(len(ids_for_tag_vacab))
