...
class CaptionPreTrainedModel(PreTrainedModel):
    ...
    def _generate_no_beam_search(
        ...
        #save all the predict logits for calculating the ppl / by Jokie
        seq_tokens_logits = []
        ...
        while cur_len < max_length:
            ...
            # seq_tokens_logits.append(next_token_logits)
        ...
        if cur_len == max_length:
            ...
            input_ids[:, -1].masked_fill_(cur_unfinished.to(dtype=torch.uint8), eos_token_ids[0]) #torch version problem by jokie
        ...
        # -----------------------------------------------add for calculating PPL tmp by Jokie-------------------------------------
        #calculate the ppl logits : [batch, gth_seq_lenth, vocab_size], gth token id [batch, seq_length]
        # seq_tokens_logits = torch.stack(seq_tokens_logits, dim=1) # [batch, generate_gth_seq_lenth, vocab_size]
        # if pad_len > 0:
        #     padding_seq_tokens_logits = seq_tokens_logits.new(batch_size, pad_len, seq_tokens_logits.shape[-1]).fill_(1e-7)
        #     padding_seq_tokens_logits[:,:,0] = 1
        #     seq_tokens_logits = torch.cat([seq_tokens_logits, padding_seq_tokens_logits], dim=1)# [batch, max_length, vocab_size]
            
        # cross_e = 0.0
        
        # for i in range(batch_size):
        #     gth = gth_ids[i] # [max_length]
        #     gth = gth[gth!=0] # [gth_seq_length]
        #     gth_seq_length = gth.shape[0]
        #     pred_logit = seq_tokens_logits[i][:gth_seq_length] # [gth_seq_length, vocab_size]
        #     try:
        #         cross_e += F.cross_entropy(pred_logit, gth, reduction='mean')
        #     except:
        #         pass
        # cross_e = cross_e / batch_size
        # # print('\ncross_e: ', cross_e)
        # ppl_per_batch = torch.exp(cross_e)

        # (batch_size, n_best, max_len), (batch_size, n_best)
        return input_ids.unsqueeze(1), sum_logprobs.unsqueeze(1)
        # return input_ids.unsqueeze(1), sum_logprobs.unsqueeze(1), ppl_per_batch
