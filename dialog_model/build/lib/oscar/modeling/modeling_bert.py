...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        Params:
            ...
            outputs: Tensor of dim [batch_size, num_queries, num_classes] with the classification logits

            targets: Tensor of dim [batch_size, num_target_boxes] (where num_target_boxes is the number of ground-truth
                           objects in the target) containing the class labels , NOTE: here num_queries == num_target_boxes
...
class BertImgModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if img_feats is not None:
            ...
            # changed by jokie since the image input is the start of the sequence
...
class BertForImageCaptioning(CaptionPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #                     29660, bias=False)
        # self.transform_lm = BertPredictionHeadTransform(config)
        # self.decoder_lm = nn.Linear(bert_embedding_weight.size(1),
        #                     bert_embedding_weight.size(0), bias=False)
        ...
        self.ids_for_tag_vacab = torch.load('/home/t-zujieliang/projects/Oscar/ids_for_tag_vacab.pth')
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask, masked_pos, masked_ids=None, 
            ...
            is_training=True, encoder_history_states=None, response_pos=None, response_ids=None, tag_pos=None, masked_tag_pos=None, masked_tag_ids=None):
        ...
        if is_training:
            ...
            # # tag_logits = tag_logits.repeat(class_logits.shape[0]).reshape(class_logits.shape[0], -1) #[class_logits.shape[0] , vocab_size]
            ...
            # # tag_vocab_mask = tag_vocab_mask[0].repeat(class_logits.shape[0]).reshape(class_logits.shape[0], -1) #[class_logits.shape[0] , vocab_size]
            # tag_logits = tag_vocab_mask * tag_logits
            # class_logits = class_logits + tag_logits
            
            #---------Masked Tag Prediction with Hungarian matching loss---------------
            masked_tag_part = sequence_output[masked_tag_pos==1, :] #[b_s* masked_tag_num, hidden_emb_size]
            transformed_output_masked_tag_part = self.transform(masked_tag_part)
            masked_tag_logits = self.decoder(transformed_output_masked_tag_part) #[b_s* masked_tag_num, vocab_size]
            tag_for_matching_logits = masked_tag_logits[:b_s * (masked_tag_logits.shape[0]//b_s),:].view(b_s, -1, self.vocab_size)

            
            masked_tag_ids = masked_tag_ids[masked_tag_ids != 0]   # remove padding masks
            tag_ids = masked_tag_ids[:b_s * (masked_tag_logits.shape[0]//b_s)]
            # Retrieve the matching between the outputs of the last layer and the targets
            
            tag_ids_for_matching = tag_ids.view(b_s, -1)
            indices = self.matcher(tag_for_matching_logits, tag_ids_for_matching)
            
            tag_for_matching_logits = tag_for_matching_logits.view(-1, self.vocab_size)
            MTP_loss = self.MLM_loss_with_Hungarian_Matching(tag_for_matching_logits, tag_ids_for_matching, indices)
            ...
            outputs = (masked_loss, class_logits, MTP_loss, tag_for_matching_logits, tag_ids_for_matching.view(-1)) + outputs[2:]

            #-------------------------Causal Language Model on Response part----------------------------------------------------

            # seq_output = outputs[0][:, :response_pos.shape[-1], :]
            # sequence_output_res = seq_output[response_pos==1, :]
            # transformed_output_lm = self.transform_lm(sequence_output_res)
            # lm_logits = self.decoder_lm(transformed_output_lm)
            # lm_labels = response_ids[response_ids != 0]   # remove padding masks
            ...
            # outputs = (masked_loss, lm_loss, class_logits, shift_logits, shift_labels) + outputs[2:]
    ...
    def prepare_inputs_for_generation(self, curr_ids, past=None):
        ...
        def _remove_elements(t, start, end):
            ...
            # print('self.od_labels_len', self.od_labels_len)
            assert t.shape == (batch_size, self.max_seq_len + self.od_labels_len)
    ...
    def generate(self, img_feats, attention_mask, masked_pos, token_type_ids=None,
            ...
            min_constraints_to_satisfy=None, use_hypo=False,gth_ids=None
            ):
    ...
    def MLM_loss_with_Hungarian_Matching(self, src_logits, targets, indices):
        ...
        # print('target_classes shape:', target_classes.shape)
        # print('target_classes[:2] :', target_classes[:2])
        # print('idx shape:', idx.shape)
        target_classes[idx] = target_classes_o
        # print('after target_classes[:2] :', target_classes[:2])
