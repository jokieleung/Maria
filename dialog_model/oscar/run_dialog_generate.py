...
from oscar.modeling.modeling_bert import BertForMultiModalDialog
...
class CaptionTensorizer(object):
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None,
            ...
            sequence_a_segment_id=0, sequence_b_segment_id=1, sequence_tag_segment_id=2): 
        if self.is_train:
        ...
        if text_b:
            ...
            # tag_start = [i for i,x in enumerate(tokens) if x=="[SEP]"][-2]
            tmp_idx = [i for i,x in enumerate(tokens) if x=="[TAG]"]
            if len(tmp_idx) is not 0:
                tag_start = tmp_idx[-1] 
        ...
        # else:    
            ...
            num_masked_tag = min(max(round(self.mask_prob * (tag_end - tag_start)), 1), self.max_masked_tokens)
        ...
        for step, ( batch) in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.logging_steps == 0:
                    logger.info("Epoch: {}, global_step: {}, lr: {:.6f}, loss: {:.4f} ({:.4f}), MTM loss: {:.4f} ({:.4f}), MLM(MRM+MCM) loss: {:.4f} ({:.4f}), " \
                        "MLM(MRM+MCM) score: {:.4f} ({:.4f}), MTM score: {:.4f} ({:.4f})".format(epoch, global_step, 
                        optimizer.param_groups[0]["lr"], loss, global_loss / global_step, MTP_loss, global_tag_loss / global_step, masked_loss, masked_global_loss / global_step,  
        ...
        with torch.no_grad():
            ...
            for step, batch in tqdm(enumerate(test_dataloader), ncols=100,desc="Testing Forward: ", total=len(test_dataloader)):
                ...
                inputs = {'is_decode': True,
                    ...
                    'bos_token_id': cls_token_id,
                    'pad_token_id': pad_token_id,
                    # 'eos_token_ids': [sep_token_id, pad_token_id, period_token_id],
                    'eos_token_ids': [sep_token_id, pad_token_id],
                    'mask_token_id': mask_token_id,
                    # for adding od labels
                    'add_od_labels': args.add_od_labels, 'od_labels_start_posid': args.max_seq_a_length,
...
def main():
    ...
    parser.add_argument("--data_dir", default='/data/data_for_oscar/reddit_oi', type=str, required=False,
                        help="The input data dir with all required files.")
    parser.add_argument("--tag_vocab_file", default='/data/data_for_oscar/reddit_oi/objects_vocab.txt', type=str, required=False,
    ...
    config_class, model_class, tokenizer_class = BertConfig, BertForMultiModalDialog, BertTokenizer
    if args.do_train:
        ...
        #---------------add by Jokie for add object tags to the vocab ------------------------
