...
class BertForMultiModalDialog(CaptionPreTrainedModel):
    """
    Bert for Multi-Modal Dialog 
    """
    def __init__(self, config):
        super(BertForMultiModalDialog, self).__init__(config)
        self.config = config
        self.bert = BertImgModel(config)
        ...
        self.ids_for_tag_vacab = torch.load('./ids_for_tag_vacab.pth')
    ...
    def _do_output_past(self, outputs):
        ...
        # 1: NOT draw attention visualization
        # return len(outputs) > 1

        # 2: draw attention visualization
        return False
