...
from coco_caption.pycocoevalcap.eval import COCOEvalCap, RedditEvalCap
...
def evaluate_on_reddit_caption(res_file, label_file, outfile=None):
    """
    res_tsv: TSV file, each row is [image_key, json format list of captions].
             Each caption is a dict, with fields "caption", "conf".
    label_file: JSON file of ground truth captions in COCO format.
    """
    assert label_file.endswith('.txt')
    
    gth = [line.decode().rstrip('\n').split('\t')[-1] for line in open(label_file,'rb')]
    ...
    redditEval = RedditEvalCap(gth, pred, 'corpus')
    # redditEval = RedditEvalCap(gth, gth, 'corpus')

    # evaluate results
    # SPICE will take a few minutes the first time, but speeds up due to caching
    redditEval.evaluate()
    result = redditEval.eval
...
# if generated result endswith TSV
# def evaluate_on_reddit_caption(res_file, label_file, outfile=None):
#     """
#     res_tsv: TSV file, each row is [image_key, json format list of captions].
#              Each caption is a dict, with fields "caption", "conf".
