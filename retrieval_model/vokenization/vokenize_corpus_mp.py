...
def processer(args, input_queue, output_queue):
    ...
    while True:
        ...
        # Change by Jokie : cached the image path rather than the image_local_id
        topks = [5]
        arg_topk = None if max(topks) == 1 else max(topks)
        if sents is not None:
            output_str = ''
            results = vokenizer.vokenize_ids(sents, attention_mask=None, topk=arg_topk)
            paths_list = results[3]

            if sentence_level:
                paths_list = [x[0] for x in paths_list]     # Only eval the first vokens.

            # print('after path:',paths_list)
            if arg_topk is None:
                paths_list = [[[img_id] for img_id in sent] for sent in paths_list]


            for paths in paths_list:                # for each sent in batch
                top_k_line = '\t'.join([os.path.split(kth_path)[-1] for kth_path in paths])
                output_str += top_k_line + '\n'
...
def segment_sent(
    ...
    for i in tqdm.tqdm(range(len(line_starts) - 1)):
        ...
        if sents_len != len(line_tokens):
            print('sents_len:',sents_len)
            print('sents:',sents)
            print('len(line_tokens):',len(line_tokens))
            print('line:',line)
            assert False
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max-img-num', type=int, default=-1,
    ...
    # Chenged by Jokie: use the whole line to retrieval the iamge
    tokens_sent_info_path = f'{args.corpus}.{args.tokenizer_name}.line'
    ...
    #     dset = h5_file.create_dataset("vokens", (num_tokens,), dtype=np.str)
