    ...
    def vokenize_sents(self, sents, topk=None):
        ...
        for sent in sents:
            input_ids.append(self.tokenizer.encode(
                ...
                # add_special_tokens=True, # sent-level should add special tokens [CLS]
    ...
    def vokenize_ids(self, input_ids, attention_mask=None, topk=None):
        ...
        def apply_model(input_ids, attention_mask, lengths):
            with torch.no_grad():
                ...
                # print('lang_output shape',lang_output.shape)
            ...
            if self.sent_level:
                ...
                # gathered_output = lang_output[:,0,:] # get the first token[CLS] to represent the sentence
                gathered_output = lang_output # get the first token[CLS] to represent the sentence By Jokie 
                # print('gathered_output shape',gathered_output.shape)
            else:
        ...
        # print('top_score.shape', top_score.shape)
        ...
        if not self.sent_level:
            ...
            #haotan If sent level, repeat the voken.
            #   Use clone() here
        ...
        if top_idxs[0].dim() == 1:
            ...
            # token level and haotan sent level
        ...
        # print('len(top_paths)', len(top_paths[0]))
        ...
        if len(sent2segs) > 0:
            print('ENTER Put back the segments of over-length sentences')
...
def load_model_and_tokenizer(load, cpu=False):
    if os.path.exists(load + '/BEST.pth.model'):
        ...
        if cpu:
            ...
            #load state dict only (By Jokie)
            # sys.path.append('/home/t-zujieliang/projects/vokenization')
            # from xmatching.model import LangModel, VisnModel, JointModel, LANG_MODELS
            # lang_layers = list(map(lambda x: -int(x), '4,3,2,1'.split(',')))     # The layers concated as the output.
            # lang_model = LangModel(64, arch='bert', layers=lang_layers,
            #                     pretrained=True, finetuning=False)
            # visn_model = VisnModel(64, arch='resnext101_32x8d',
            #                     pretrained=True, finetuning=False)
            
            # joint_model = JointModel(lang_model, visn_model)
            # joint_model.load_state_dict(torch.load(load + '/BEST.pth'))
